...
router.get('/:title', async (req, res) => {
  ...
  if(regex.test(title)){
    ...
    const {rows} = await db.query('SELECT * FROM entries WHERE LOWER(header)=LOWER($1)', [header]);
    if(rows.length){
...
router.get('/page/:page', async (req, res) => {
  ...
  if(typeof offset == 'number' && offset >= 0){
    let {rows} = await db.query('SELECT * FROM (SELECT *, ROW_NUMBER() OVER() FROM entries) x ORDER BY ROW_NUMBER DESC OFFSET $1 ROWS FETCH FIRST $2 ROWS ONLY', [offset, entriesPerPage]);
    rows.map((row, index) => {
      let url = row.header;
...
router.post('/new', verify, async (req, res) => {
  ...
  if(header.trim() != '' &&
     ...
     category.trim() != '' &&
     content.trim() != ''){
    const {rows} = await db.query('INSERT INTO entries VALUES($1, $2, $3, $4, $5, $6)', [header, subheader, category, content, author, dateStr]);
